...
from api.gemini import translate_lyrics, explain_song_meaning, get_similar_songs, get_lyrics_by_gemini, is_configured as gemini_configured
...
def translate():
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': f'Failed to translate lyrics: {str(e)}'
        }), 500
...
def explain_meaning():
    ...
    try:
        ...
        logger.info(f"Meaning explanation completed using: {result.get('api_used', 'unknown')}")
        
        # If Gemini returned an error and we have lyrics, we could try to provide a basic analysis
        if result.get('status') == 'error' and 'gemini_error' in result.get('api_used', ''):
...
def similar_songs():
    ...
    try:
        ...
        logger.info(f"Similar songs search completed using: {result.get('api_used', 'unknown')}")
        return jsonify(result)
    except Exception as e:
        logger.exception(f"Error finding similar songs: {str(e)}")
        return jsonify({
            'status': 'error',
            'message': f'Failed to get similar songs: {str(e)}'
        }), 500
...
def debug_gemini_status():
    ...
    logger.info(f"Debug request for Gemini status: configured={is_configured}, key_present={api_key_present}")
    
    return jsonify({
        'status': 'success',
        'gemini_configured': is_configured,
        'api_key_present': api_key_present,
        'message': 'Gemini API is properly configured' if is_configured else 'Gemini API is not configured'
    })
